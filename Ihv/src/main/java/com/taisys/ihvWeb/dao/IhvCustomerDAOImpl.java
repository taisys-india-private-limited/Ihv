...
public class IhvCustomerDAOImpl implements IhvCustomerDAO {
	...
	public boolean createCustomer(IhvCustomer wallet) {
		try {
	...
	public IhvCustomer getCustomerByMobile(String mobileNo) {
		try {
			Criteria criteria = getSession().createCriteria(IhvCustomer.class);
			criteria.add(Restrictions.eq("mobileNo", mobileNo));
			List<IhvCustomer> wallets = criteria.list();
			if (wallets.isEmpty()) {
	...
	public boolean updateCustomer(IhvCustomer wallet) {
		try {
